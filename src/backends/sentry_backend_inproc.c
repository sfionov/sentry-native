...
fill_frame(const sentry_frame_info_t *frame_info, void *arg)
{
    ...
    sentry_value_set_by_key(frame, "package",
        sentry_value_new_string(frame_info->object_name));
    sentry_value_set_by_key(frame, "function",
        sentry_value_new_string(frame_info->symbol));
}
    ...
    const struct signal_slot *sig_slot, const sentry_ucontext_t *uctx)
{
    ...
    for (size_t i = 0; i < frame_count; i++) {
        ...
        sentry_value_set_by_key(frame, "instruction_addr",
            sentry__value_new_addr((uint64_t)ip));
